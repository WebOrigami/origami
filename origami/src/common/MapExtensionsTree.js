 ...
 * the inner tree's values. If an inner extension is specified, only apply the
 * transformation to keys in the inner tree that end with that extension. If an
 * outer extension is specified, change the extension on the exposed outer keys
 * to that extension.
 */
export default class MapExtensionsTree extends MapKeysValuesTree {
  constructor(treelike, mapFn, options = {}) {
    super(treelike, mapFn, options);
  ...
  async innerKeyForOuterKey(outerKey) {
    ...
    return basename
      ...
      : !this.extensionMatchesOnly ||
        (await Tree.isKeyForSubtree(this.tree, outerKey))
      ? outerKey
      : undefined;
  }
  ...
  async outerKeyForInnerKey(innerKey) {
    ...
    return basename
      ...
      : !this.extensionMatchesOnly ||
        (this.deep && (await Tree.isKeyForSubtree(this.tree, innerKey)))
      ? innerKey
      : undefined;
  }
}
