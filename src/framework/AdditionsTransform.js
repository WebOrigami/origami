    ...
    async keyAdded(key, options, existingKeys) {
      ...
      if (isChildAdditionKey(key)) {
        ...
        if (addition) {
          ...
          // REVIEW: We expose all the keys in the child additions so that, if a
          // child addition itself has child additions, the top-level graph will
          // expose those. That said, using allKeys() for this purpose means
          // that virtual keys generated by formulas will also be exposed, even
          // though the top-level graph will also do the work to generate those
          // keys. So using allKeys() isn't efficient.
