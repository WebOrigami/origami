...
describe("FunctionGraph", () => {
  test("constructs an explorable function", async () => {
  ...
  test("get executes if no keys are provided", async () => {
    const graph = new FunctionGraph(() => `Hello.`, ["a", "b", "c"]);
    const result = await graph.get(undefined);
    assert.equal(result, "Hello.");
  });

  test("get curries a multi-argument function", async () => {
  ...
  test("can traverse more keys than its function accepts", async () => {
    const obj = {
      x: {
        y: {
          z: 1,
  ...
  test("can traverse a function with a spread parameter", async () => {
